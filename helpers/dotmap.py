...
class DotMap(MutableMapping, OrderedDict):
    def __init__(self, *args, **kwargs):
        ...
        self._prevent_method_masking = kwargs.pop(
            ...
            for k, v in src:
                ...
                if isinstance(v, dict):
                    ...
                    if idv in trackedIDs:
                        ...
                        trackedIDs[idv] = v
                        v = self.__class__(
                            v, _dynamic=self._dynamic, _prevent_method_masking=self._prevent_method_masking, _trackedIDs=trackedIDs)
                if type(v) is list:
                    l = []
                    for i in v:
                        n = i
                        if isinstance(i, dict):
                            ...
                            if idi in trackedIDs:
                                ...
                                trackedIDs[idi] = i
                                n = self.__class__(
                                    i, _dynamic=self._dynamic, _prevent_method_masking=self._prevent_method_masking)
                        l.append(n)
                    v = l
                self._map[k] = v
        if kwargs:
...
reserved_keys = {i for i in dir(DotMap) if not i.startswith(
    '__') and not i.endswith('__')}
